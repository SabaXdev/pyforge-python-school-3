...
def create_molecule(db: Session, molecule: schemas.MoleculeCreate):
    logger.info("Creating molecule with ID %d and name %s",
                molecule.mol_id, molecule.name
                )
...
def get_all_molecules(db: Session, skip: int = 0, limit: int = 100):
    logger.info("Fetching all molecules with skip=%d and limit=%d",
                skip, limit)
...
def search_molecules(db: Session, substructure_smile: str):
    ...
    if not substructure:
        logger.error("Invalid substructure SMILES: %s",
                     substructure_smile)
