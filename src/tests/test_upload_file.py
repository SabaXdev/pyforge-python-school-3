...
@pytest.mark.parametrize("file_content, expected_status_code, expected_response", [
    ("5 CH4\n6 CO\n", 200,  # Valid ID and SMILES format
     {'content': 'Molecule/Molecules added successfully.'}
     ),
    ("invalid_id invalid_smiles", 400,  # Invalid ID and SMILES should result in a 400 Bad Request
     {'detail': "invalid literal for int() with base 10: 'invalid_id'"})
])
def test_upload_file(file_content, expected_status_code, expected_response, db_session):
